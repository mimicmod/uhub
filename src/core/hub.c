...
void hub_set_variables(struct hub_info* hub, struct acl_handle* acl)
{
	...
	hub->command_banner = adc_msg_construct(ADC_CMD_ISTA, 100 + strlen(PRODUCT_STRING));
...
#define STATUS(CODE, MSG, FLAG, RCONTIME, REDIRECT) case status_ ## MSG : set_status_code(level, CODE, code); text = cfg->MSG; flag = FLAG; reconnect_time = RCONTIME; redirect = REDIRECT; break
	switch (msg)
	{
		...
		STATUS(43, msg_inf_error_nick_missing, 0, 0, 1);
		STATUS(43, msg_inf_error_nick_multiple, 0, 0, 1);
		STATUS(21, msg_inf_error_nick_invalid, 0, 0, 1);
		STATUS(21, msg_inf_error_nick_long, 0, 0, 1);
		STATUS(21, msg_inf_error_nick_short, 0, 0, 1);
		STATUS(21, msg_inf_error_nick_spaces, 0, 0, 1);
		STATUS(21, msg_inf_error_nick_bad_chars, 0, 0, 1);
		STATUS(21, msg_inf_error_nick_not_utf8, 0, 0, 1);
		STATUS(22, msg_inf_error_nick_taken, 0, 0, 1);
		STATUS(21, msg_inf_error_nick_restricted, 0, 0, 1);
		STATUS(43, msg_inf_error_cid_invalid, "FBID", 0, 1);
		STATUS(43, msg_inf_error_cid_missing, "FMID", 0, 1);
		STATUS(24, msg_inf_error_cid_taken, 0, 0, 1);
		STATUS(43, msg_inf_error_pid_missing, "FMPD", 0, 1);
		STATUS(27, msg_inf_error_pid_invalid, "FBPD", 0, 1);
		STATUS(31, msg_ban_permanently, 0, 0, 1);
		STATUS(32, msg_ban_temporarily, "TL600", 600, 1); /* FIXME: Proper timeout? */
