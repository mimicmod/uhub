 ...
 * Copyright (C) 2007-2016, Jan Vidar Krey
 *
 * THIS FILE IS AUTOGENERATED - DO NOT MODIFY
 * Created 2016-04-02 19:21, by config.py
 */
...
void config_defaults(struct hub_config* config)
{
	...
	config->failover_redirect_addr = hub_strdup("");
...
static int apply_config(struct hub_config* config, char* key, char* data, int line_count)
{
	...
	if (!strcmp(key, "failover_redirect_addr"))
	{
		if (!apply_string(key, data, &config->failover_redirect_addr, (char*) ""))
		{
			LOG_ERROR("Configuration parse error on line %d", line_count);
			return -1;
		}
...
void free_config(struct hub_config* config)
{
	...
	hub_free(config->failover_redirect_addr);
...
void dump_config(struct hub_config* config, int ignore_defaults)
{
	...
	if (!ignore_defaults || strcmp(config->failover_redirect_addr, "") != 0)
		fprintf(stdout, "failover_redirect_addr = \"%s\"\n", config->failover_redirect_addr);
